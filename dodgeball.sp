...
displaySpeed(String:clientName[32], Float:speed)
{
	PrintToChatAll("\x04%s\x01 died to a rocket travelling at \x04%.0f\x01 MPH", clientName, calculateSpeed(speed));
}

public void displaySpeedOnScreen(Float:speed)
{
	SetHudTextParams(0.5, 0.85, 1.0, 255, 50, 50, 255);
	
	for (new client = 1; client <= MaxClients; client++)
	{
		if(IsClientConnected(client) && IsClientInGame(client))
		{
			ShowHudText(client, 1, "%.0f MPH", calculateSpeed(speed));
		}
	}
}
...
public Action:OnPlayerDeath(Handle:hEvent, String:strEventName[], bool:bDontBroadcast)
{
	...
	GetClientName(iVictim, clientName, sizeof(clientName));
	...
	if (IsValidClient(iVictim))
	{
		...
		if (iIndex != -1)
		{
			...
			displaySpeed(clientName, fSpeed);
		}
	}
...
HomingRocketThink(iIndex)
{
	...
	else if ((iDeflectionCount > g_iRocketDeflections[iIndex]))
	{
		...
		displaySpeedOnScreen(g_fRocketSpeed[iIndex]);
	}
	else
	{
